...
module MemMapBuf =
  ...
  module Unsafe =
    ...
    val truncate: MemMapBuf -> PtrInt -> Alt<unit>
  ...
  val accessFun: MemMapBuf -> (nativeptr<byte> -> 'x) -> Alt<'x>
  ...
  val accessJob: MemMapBuf -> (nativeptr<byte> -> #Job<'x>) -> Alt<'x>

  /// Allocates space from the end of the buffer and return offset to the start
  /// of the allocated space.
  val append: MemMapBuf -> align: int -> size: int -> Alt<PtrInt>
